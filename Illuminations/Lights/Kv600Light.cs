...
{
    public class Kv600Light : Light
    {
        IKv600Serial serial;

        public Kv600Light(IKv600Serial serial, string name, byte channel, int brightness, int maxBrightness, bool isInterlocked, string interlockGroup) : base(name, channel, brightness, maxBrightness, isInterlocked, interlockGroup)
        {
            this.serial = serial;
        }
        ...
        public override void SetBrightness(int brightness)
        {
            ...
            if (brightness == 1)
            {
                brightness = 2; // better hallucination.
            }
