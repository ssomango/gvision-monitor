...
{
    public class LiveSettingJigOffsetProcessor : ILiveFrameProcessor
    {
        private int index = 0;
        private readonly SettingJigCalibrationRepository settingJigRepository = SettingJigCalibrationRepository.Instance;

        public LiveSettingJigOffsetProcessor(ECamera cameraType, HSmartWindowControlWPF hSmartWindowControlWpf)
        {
        ...
        private CalibrationResult calculateOffset(HObject image)
        {
            ...
            CalibrationResult result = CalibrationEngine.CalibrationEngineBase(image, teaching.Roi, teaching.Threshold, teaching.MinSize, teaching.MaxSize, teaching.Similarity, teaching.ShapeType, teaching.StandardType, ECalibration.SettingJig, this.CameraType);
